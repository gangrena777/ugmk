...
function get_object_by_id($connect, $id){
	...
	$field = mysqli_fetch_assoc($result);
		...
		$query_video = "SELECT * FROM video WHERE object_id = $id";
		$res_vid = mysqli_query($connect, $query_video);
		if($res_vid){

			while($file_video = mysqli_fetch_assoc($res_vid)){

					$data['video_idsz'][] = $file_video;
			}
		}
...
function addvideoDB($connect, $object_id, $file_path){
	...
	$t = "INSERT INTO video (  name, `path`, object_id ) VALUES ('%s', '%s', '%s')";
	$query = sprintf($t, 
						 mysqli_real_escape_string($connect, $name),
						  mysqli_real_escape_string($connect, $path),
						  mysqli_real_escape_string($connect, $object_id),
