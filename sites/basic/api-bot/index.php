											       ...
											       ///   если прислали видео
														...
														$res = sendTelegram(
															...
															if ($res['ok']) {
																...
																if ($path = getTGApiFile('video',$res['result']['file_path'], false)) {

                                                                    $add_video = addvideoDB($connect,$user_data['object_id'], $path );
                                                                    if($add_video){
					                                    ...
					                                    if($res){
								                    		...
								                    		$buttons = get_object_buttons_by_id($connect, $user_data['object_id']);
								                    			...
								                    			if( isset($obj['photo_idsz']) && !empty($obj['photo_idsz'])){
								                    		        ...
								                    		        ////отправляем все видео по обьекту
								                    			if( isset($obj['video_idsz']) && !empty($obj['video_idsz'])){

								                    			    foreach ($obj['video_idsz'] as $id => $value) {


								                    					$url_video = $_SERVER['DOCUMENT_ROOT'].'/web/'.$value['path']; 

								                    					  file_put_contents(__DIR__ . '/video', print_r($url_video, true));

                                                                        $caption = "видео_".$value['name'];
                                                                        ...
                                                                        sendTelegram(
																			'sendVideo', 
																				array(
																					 	'chat_id' => $data['message']['chat']['id'],
																	                    'video' => curl_file_create($url_video), 
																	                    'caption' => $caption, 
																	                    'parse_mode' => "html"
																				)
																		);	
		            ...
		            if($id){
			                 ...
			                 $buttons = get_object_buttons_by_id($connect, $id);
									...
									if( isset($obj['photo_idsz']) && $obj['photo_idsz'] !=='' ){
										  ...
										  // file_put_contents(__DIR__ . '/photosz.txt', print_r($obj['photo_idsz'], true));
													...
													foreach ($obj['photo_idsz'] as $id => $value) {
								        				...
								        				$url = $_SERVER['DOCUMENT_ROOT'] .'/web/'.$value['path']; 
								        					...
								        					file_put_contents(__DIR__ . '/path_photo.txt', 							print_r($url, true));
								     ...
								     ///video
									if( isset($obj['video_idsz']) && $obj['video_idsz'] !=='' ){
													...
													foreach ($obj['video_idsz'] as $id => $value) {
								        				...
								        				$url = $_SERVER['DOCUMENT_ROOT'] .'/web/'.$value['path']; 
								        					...
								        					file_put_contents(__DIR__ . '/path_vid.txt', 							print_r($url, true));
								    ...
								    if( isset($obj['doc_idsz']) && $obj['doc_idsz'] !=='' ){
			                    			...
			                    			file_put_contents(__DIR__ . '/path_doc.txt', print_r($obj['doc_idsz'], true));
											foreach ($obj['doc_idsz'] as $id => $value) {
