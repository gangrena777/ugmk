...
class Objects extends \yii\db\ActiveRecord
{
    ...
    public function UploadVideo($id)
    {
        ...
        {   
            ...
            {
                 $path = 'upload/video/'. $vid->baseName . '.' . $vid->extension;
                 if($vid->saveAs($path)){

                      $this->SaveDbVideo($vid, $path, $id);
                }
            }
    ...
    {
        return $this->hasMany(Video::className(), [ 'object_id' => 'id']);
    }
    ...
    public function SaveDbDocs($doc, $path, $id){
       ...
       public function SaveDbVideo($video, $path, $id){
              $VIDEO = new Video();

                    $values = [
                        'name' => $video->baseName . '.' . $video->extension,
                        //'path' => substr($path,2),
                        'path' => $path,
                        'object_id' =>$id
                    ];
                    $VIDEO->attributes = $values;
                    $VIDEO->save();
                     unset($model->video);
