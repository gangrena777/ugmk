...
<?php
    ...
    if(isset($photos) && !empty($photos)){
        ...
        foreach ($photos as  $photo)
        {
         ...
         $photos_box .= '<div  class="view_photo">
           <a href="'.Url::to('@web/web/'.$photo->path).'" rel="lightbox">'.Html::img('@web/web/'.$photo->path).'</a>
           <a  class ="delete_photo"  href="'.$url_delete.'"   data-id ="'.$photo->id.'">&#x2715</a>
         </div>
         ';
        }
    }
    ...
    if(isset($video) && !empty($video)){
        ...
        {
         $url_delete = Url::toRoute(['objects/deletephoto','id_video'=>$vid->id]);
         $video_box .= '<div  class="view_video">
           


                          <video width="200" height="100" controls="controls" poster="'.Url::to('@web/web/'.$vid->path).'">
           ...
           <a  class ="delete_video"  href="'.$url_delete.'"   data-id ="'.$vid->id.'">&#x2715</a>
         </div>
         ';
        }
    }
    ...
    if(isset($video) && !empty($video))
        {
                    ...
                    echo'<div class ="video">
                          <label class="control-label fotos">Видео обьекта :</label> ';
                    echo'<div class="video_box">';
                    echo $video_box;
                    echo'</div>';
                    echo'</div>';
                    echo $form->field($model, 'video[]')->label('Добавить еще ...')->fileInput(['multiple' => true,'class' => 'video_del']);
              
        }else{                
             
              echo $form->field($model, 'video[]')->fileInput(['multiple' => true]);
        }
    ?>
